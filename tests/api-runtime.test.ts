...
test("graph rejects an unknown mode with a non-cacheable 400 response", async () => {
  ...
  const response = await get(new Request("https://syntag.test/api/graph?discipline=education&mode=unknown"));
...
test("graph returns 404 when a published discipline is not found", async () => {
  ...
  const response = await get(new Request("https://syntag.test/api/graph?discipline=unknown"));
...
test("graph exposes database unavailability as a recoverable 503 response", async () => {
  ...
  const response = await get(new Request("https://syntag.test/api/graph?discipline=education"));
...
test("search rejects an empty query with a non-cacheable 400 response", async () => {
  ...
  const response = await get(new Request("https://syntag.test/api/search?q=%20%20"));
...
test("search accepts concept and field as public searchable entity types", async () => {
  ...
  assert.equal((await get(new Request("https://syntag.test/api/search?q=identity&type=concept"))).status, 200);
  assert.equal((await get(new Request("https://syntag.test/api/search?q=identity&type=field"))).status, 200);
...
test("search rejects unknown entity types with the current contract message", async () => {
  ...
  const response = await get(new Request("https://syntag.test/api/search?q=identity&type=framework"));
...
test("search exposes database unavailability as a recoverable 503 response", async () => {
  ...
  const response = await get(new Request("https://syntag.test/api/search?q=identity"));
