...
export function SearchBox({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={`search-box${compact ? " search-box--compact" : ""}`}>
    ...
    {open && <div className="search-box__overlay" role="dialog" aria-label="Search Syntag"><div className="search-box__panel"><div className="search-box__input"><input ref={inputRef} value={query} onChange={(event) => updateQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); if (event.key === "Enter") { event.preventDefault(); submitSearch(); } }} placeholder="Search theories, scholars, works, topics, concepts, and fields" /><Link href={`/search?q=${encodeURIComponent(query)}`} onClick={() => setOpen(false)}>View all</Link></div>
