...
export function EntityArticle({ kind, title, summary, breadcrumb, overview, links, children }: { kind: string; title: string; summary?: string | null; breadcrumb: Array<{ label: string; href?: string }>; overview?: string | null; links?: Array<{ label: string; href: string; description?: string | null }>; children?: React.ReactNode }) {
  ...
  return <article className="entity-page content-layout"><Breadcrumbs path={[{ label: "Home", href: "/" }, ...breadcrumb, { label: title }]} /><header className="entity-page__hero">{isScholar ? <div className="entity-page__identity"><div className="monogram" aria-label={`${title} monogram`}>{initialsForTitle(title)}</div><div><span>{kind}</span><h1>{title}</h1>{summary && <p>{summary}</p>}</div></div> : <><span>{kind}</span><h1>{title}</h1>{summary && <p>{summary}</p>}</>}</header><ProseSection title="Overview">{overview || summary || "Editorial content for this entry is being prepared."}</ProseSection>{children}<ContentAd placement="in-article" /><RelatedLinks heading={`Related ${kind.toLowerCase()} entries`} links={links || []} /><ContentAd placement="bottom" /><SourceBlock sources={[{ text: "Source-level editorial verification is pending for this entry.", level: "L3_pending" }]} /><Disclaimer /></article>;
}
